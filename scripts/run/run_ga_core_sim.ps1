# Run GA core smoke simulation with Vivado 2023.2 on this workstation.
# Fix Tcl conflict from system/Python Tcl and a Tcl 8.5 path-normalization issue
# under C:\Users\huazh\Desktop by using a local copy of Vivado's Tcl library.
# Call unwrapped executables directly because Vivado's *.bat loader overwrites TCL_LIBRARY.
...
& $xvlog -sv -f filelist_tb.f
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $xelab tb_ga3b_ga_core_smoke -s tb_ga3b_ga_core_smoke_sim
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
& $xsim tb_ga3b_ga_core_smoke_sim -runall
exit $LASTEXITCODE
