...
本项目在正点原子领航者 V2（Zynq-7020，芯片型号为`xc7z020clg400-2`）上实现三体问题初始条件的遗传搜索、实板 UART/DMA 加速和 Web 轨迹展示。当前可部署主线是 **Profile-5 Pure3**：在三颗等质量天体、质心与总动量为零的约束下，搜索具有长时生存、近掠、纠缠或近周期回归特征的二维初始条件。
...
它们不等价于无限稳定轨道或严格数学周期解，只是基于观测现象的物理建模和近似。
...
- 保留但未作为当前验收 bitstream 部署：三太阳加测试行星的受限四体“恒纪元”原型 RTL。它是科学目标和后续方向。曾尝试布局布线，但因占用资源大幅超过 Zynq-7020 PL 以及时序严重违例，已经不在工程优化范围之内，所以未能在现有板上实现。
- Web 同步搜索上限为 131072 步。1M steps 的更长窗口已通过直接 UART 验证；2,097,152 steps 会耗尽现有 DMA 超时，需物理冷启动恢复。
