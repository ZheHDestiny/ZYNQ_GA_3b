...
#define GA3B_PROFILE_PURE3_HIFI_SYMPLECTIC 0x00000004u
#define GA3B_PROFILE_PURE3_HIFI_LEAPFROG   0x00000005u
...
#define GA3B_PURE3_TASK_WORDS    (GA3B_TASK_HEADER_WORDS + GA3B_PURE3_GENE_COUNT * GA3B_GENE_BOUND_WORDS) //31 words
...
static inline uint32_t ga3b_q16_from_int(int32_t x) {    //integer to Q16 fixed-point conversion
    return (uint32_t)(x << 16);
}

static inline size_t ga3b_make_pure3_task(uint32_t *task_words, //SW serialization, interaction data with PL Accelerator
                                          uint16_t max_gen,
    ...
    size_t w = 0;
    task_words[w++] = GA3B_MAGIC_TASK;   //task name
    task_words[w++] = GA3B_TASK_WORD1_PURE3_RF;   //gene_count,pop_size
    task_words[w++] = (uint32_t)max_gen;          //max_gen
    task_words[w++] = steps_limit;                //steps_limit
    task_words[w++] = ((uint32_t)crossover_rate_q16 << 16) | mutation_rate_q16; //crossover_rate & mutation_rate
    task_words[w++] = seed0;                                                    //random seed0
    task_words[w++] = seed1;                                                    //random seed1
    for (size_t i = 0; i < GA3B_PURE3_GENE_COUNT; ++i) { //8个gene，每个gene有3个参数：min, max, scale，24words
