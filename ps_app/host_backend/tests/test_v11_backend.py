...
from ga3b_reference import replay_trajectory, score_trajectory, select_display_window
...
class FakeService:
    ...
    def search(self, req, profile="survival", candidate_count=None):
        result=SearchResult.from_uart_line(RESULT_LINE).to_dict();result["trajectory"]=replay_trajectory([int(x,16) for x in result["genes_raw"]],req.steps)
...
def test_result_parser_and_q16():
    ...
    assert result.to_dict()["genes"][0] < 0
...
def test_api_and_static_page(tmp_path):
    ...
    assert replay.get_json()["execution_target"] == "pc_rtl_replay"
    assert "display_trajectory" in replay.get_json()


def test_four_physics_profiles_and_custom_limits():
    trajectory=replay_trajectory([0x00010000,0,0,0,0xFFFF0000,0,0,0],32)
...
def test_event_highlight_window_is_continuous_and_preserves_absolute_steps():
    ...
    full=replay_trajectory(genes,8192)
